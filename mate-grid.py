...
from sh import gsettings, xdotool, xwininfo, grep, awk, wmctrl
from os import path
...
def get_monitor_config():
    ...
    if(not path.exists(monitorsxml_path)):
        print("Error: monitors.xml does not exixsts run monitor config utility from the control pannel")
        return None, None
...
def get_panel_size():
    panel_size = gsettings("get", 
                           "org.mate.panel.toplevel:/org/mate/panel/toplevels/toplevel_4/",
                           "size")
...
def get_window_info():
    ...
    # equivalent to `xwininfo -id $window | grep "Absolute upper-left X" | awk '{print $4}'`
    ...
    window_obj = namedtuple("Window", ['position', 'width', 'window_border_offset', 'window_title_offset'])

    return window_obj(int(position), int(width), int(window_border_offset), int(window_title_offset))
...
def remove_maximized_state():
    ...
    call(['wmctrl', '-r', ':ACTIVE:','-b', 'remove,maximized_vert,maximized_horz'])


def restore_maximized_state():
    #wmctrl('-r', ':ACTIVE:','-b', 'add,maximized_vert,maximized_horz')
    call(['wmctrl','-r', ':ACTIVE:','-b', 'add,maximized_vert,maximized_horz'])
...
def change_monitor():
    ...
    if current_window.position < left_monitor.width:
        ...
        # check that the window position is still on the visible area of the second monitor
        # if not put the window on top left corner
...
if __name__ == '__main__':
    parser = ArgumentParser(description='spectacle like program for resizing and move windows in the mate desktop environment',
                            prog='mate-grid') 
    parser.add_argument('-ml',  '--move-left', action='store_true', help='moves the window to the left half')
    parser.add_argument('-mr',  '--move-right', action='store_true', help='moves the window to the right half')
    parser.add_argument('-mt',  '--move-top', action='store_true', help='moves the window to the right half')
    parser.add_argument('-mb',  '--move-bottom', action='store_true', help='moves the window to the right half')
    parser.add_argument('-mtl', '--move-top-left', action='store_true', help='moves the window to the top left quarter')
    parser.add_argument('-mtr', '--move-top-right', action='store_true', help='moves the window to the top right quarter')
    parser.add_argument('-mbl', '--move-bottom-left', action='store_true', help='moves the window to the bottom left quarter')
    parser.add_argument('-mbr', '--move-bottom-right', action='store_true', help='moves the window to the bottom right quarter')
    parser.add_argument('-cm',  '--change-monitor', action='store_true', help='moves the window between monitors')
